...
int IOHelper::init(int argc, char **argv)
{
    ...
    /*
  // -------------------- read mesh
  if (!IO::read_mesh(mesh, argv[optind], m_ropt))
  {
  ...
  std::cout << "  is binary: "
            << (m_ropt.check(IO::Options::Binary) ? " yes\n" : " no\n");
  std::cout << "  byte order: ";
  if (m_ropt.check(IO::Options::Swap))
    std::cout << "swapped\n";
  else if (m_ropt.check(IO::Options::LSB))
    std::cout << "little endian\n";
  else if (m_ropt.check(IO::Options::MSB))
    std::cout << "big endian\n";
  else
    std::cout << "don't care\n";
  ...
  std::cout << "  provides VertexNormal"
            << ( // strange layout for doxygen
                   m_ropt.check(IO::Options::VertexNormal)
                       ? ": yes\n"
                       : ": no\n");
  CHKROPT(VertexColor);
  ...
  std::cout << "  use binary: "
            << (m_wopt.check(IO::Options::Binary) ? " yes\n" : " no\n");
  std::cout << "  byte order: ";
  if (m_wopt.check(IO::Options::Swap))
    std::cout << "swapped\n";
  else if (m_wopt.check(IO::Options::LSB))
    std::cout << "little endian\n";
  else if (m_wopt.check(IO::Options::MSB))
    std::cout << "big endian\n";
  else
    std::cout << "don't care\n";

  std::cout << "  write VertexNormal"
            << (m_wopt.check(IO::Options::VertexNormal) ? ": yes\n" : ": no\n");
  CHKWOPT(VertexColor);
  ...
  if ( !IO::write_mesh( mesh, argv[optind+1], m_wopt ) )
  {
